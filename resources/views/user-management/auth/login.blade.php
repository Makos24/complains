...
<form class="pt-3" action="{{ route('auth.user.login') }}" method="post">
    ...
    <div class="form-group">
        <input type="text" name="{{ config('laravel_user_management.auth.username') }}" class="form-control form-control-lg" placeholder="{{ __('trans.placeholders.username') }}" required>
    </div>
    <div class="form-group">
